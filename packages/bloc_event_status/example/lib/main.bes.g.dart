...
extension $TodoBlocEmitterX on Emitter<TodoState> {
  ...
  void success<TData extends Object?, T extends TodoEvent>(
          T event, TodoState state,
          [TData? data]) =>
      _emitEventStatus(
          event,
          SuccessTodoEventStatus<TData>(
            data,
          ),
          state);

  void failure<T extends TodoEvent>(
          T event, TodoState state, Exception error) =>
      _emitEventStatus(
          event,
