...
void main() {
  group('BlocBuilderWithCustomEventStatus', () {
    ...
    Widget testBuilder(
      ...
      return Text(
        'Event: ${event?.data ?? "None"}, Status: ${status?.name ?? "None"}, State: $state',
      );
    }
        ...
        MaterialApp(
          home: Scaffold(
            body: BlocProvider.value(
              value: testBloc,
              child: BlocBuilderWithCustomEventStatus<TestBloc, TestEvent,
                  EventA, int, TestStatus>(
                builder: testBuilder,
              ),
            ),
          ),
    ...
    testWidgets('rebuilds with correct event and status when status changes',
      ...
      expect(
        find.text('Event: event1, Status: loading, State: 0'),
        findsOneWidget,
      );
      ...
      await tester.pumpAndSettle();
      expect(
        find.text('Event: event2, Status: success, State: 0'),
        findsOneWidget,
      );
    });
    ...
    testWidgets('rebuilds when filter returns true', (tester) async {
        ...
        MaterialApp(
          home: Scaffold(
            body: BlocProvider.value(
              ...
              child: BlocBuilderWithCustomEventStatus<TestBloc, TestEvent,
                  EventA, int, TestStatus>(
                eventFilter: (event) => event.data == 'allowed',
                builder: testBuilder,
              ),
            ),
          ),
      ...
      await tester.pumpAndSettle();
      expect(
        find.text('Event: allowed, Status: loading, State: 0'),
        findsOneWidget,
      );
    });
    ...
    testWidgets('does not rebuild when filter returns false', (tester) async {
      ...
      expect(
        find.text('Event: blocked, Status: success, State: 0'),
        findsNothing,
      );
    });
    ...
    testWidgets('does not rebuild when buildWhen returns false',
      ...
      expect(
        find.text('Event: event3, Status: failure, State: 0'),
        findsNothing,
      );
    });
    ...
    testWidgets('uses provided bloc parameter if available', (tester) async {
      ...
      expect(
        find.text('Event: ignored, Status: success, State: 0'),
        findsNothing,
      );
    });
    ...
    testWidgets('updates subscription and rebuilds when bloc instance changes',
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: BlocBuilderWithCustomEventStatus<TestBloc, TestEvent, EventA,
                int, TestStatus>(
              bloc: blocA,
              builder: testBuilder,
            ),
      ...
      await tester.pumpAndSettle();
      expect(
        find.text('Event: fromA, Status: loading, State: 0'),
        findsOneWidget,
      );
      ...
      expect(find.text('Event: None, Status: None, State: 0'), findsOneWidget);
      expect(
        find.text('Event: ignored, Status: failure, State: 0'),
        findsNothing,
      );
      ...
      await tester.pumpAndSettle();
      expect(
        find.text('Event: fromB, Status: success, State: 0'),
        findsOneWidget,
      );
    });
    ...
    testWidgets('only listens to and rebuilds for the specified event subtype',
      ...
      await tester.pumpAndSettle();
      expect(
        find.text('Event: sub, Status: loading, State: 0'),
        findsOneWidget,
      );
    ...
    testWidgets(
      ...
      await tester.pumpAndSettle();
      expect(
        find.text('Event: event1, Status: success, State: 0'),
        findsOneWidget,
      );
    ...
    testWidgets('respects filter on initial build (initState)', (tester) async {
      ...
      testBloc.emitEventStatus(eventA, TestStatus.success);
